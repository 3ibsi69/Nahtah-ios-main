...
export default function UpdateEvent() {
  ...
  useEffect(() => {
    ...
    setSelectedBarberImage(barberImg);
    if (hairstyles === "حلاقة ذقن") {
      setSelectedHaircuts([2]);
    } else if (hairstyles === "حلاقة شعر") {
      setSelectedHaircuts([3]);
    } else if (hairstyles === "حلاقة ذقن و شعر") {
      setSelectedHaircuts([1]);
    } else if (hairstyles === "حلاقة ذقن و شعر ") {
      setSelectedHaircuts([1]);
    }
    setSelectedValue(time);
    setSelectedDate(dateP);
  }, [barberName, time, dateP, hairstyles, status, barberId, barberImg]);
  ...
  const UpdateAppointment = async () => {
    if (
      ...
      selectedValue === time &&
      fetchTimeOptions(selectedDate) === dateP &&
      selectedHaircuts === hairstyles
    ) {
      ...
      if (AfterMidnightTimes.some((option) => option.time === selectedValue)) {
        ...
        if (user) {
          await axios
            ...
            .then(async (res) => {
              if (res.data.alert) {
                alert("الحدث مع هذا العامل والوقت موجود بالفعل");
              }
              if (res.data.msg) {
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
      <View style={styles.mainContainer}>
        {/* Pickers Section */}
        <Text style={styles.title}>تعديل موعد</Text>
